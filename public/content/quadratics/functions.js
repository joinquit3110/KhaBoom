...
var StepFunctions = (() => {
  ...
  // content/quadratics/functions.ts
  var functions_exports = {};
  __export(functions_exports, {
    directrix: () => directrix,
    intro4: () => intro4,
    introChart: () => introChart,
  ...
  var Line = class _Line {
    ...
    perpendicular(p) {
      const q2 = this.line.project(p);
      if (Point.equals(p, q2)) return new _Line(q2, q2.add(this.perpendicularVector.scale(this.length / 2)));
      return new _Line(q2, p);
    }
  ...
  var Segment = class _Segment extends Line {
    ...
    project(p) {
      const a = Point.difference(this.p2, this.p1);
      const b = Point.difference(p, this.p1);
      const q2 = clamp(Point.dot(a, b) / this.lengthSquared, 0, 1);
      return this.p1.add(a.scale(q2));
    }
  ...
  var Polygon = class _Polygon {
    ...
    static collision(p1, p2) {
      if (p1.points.some((q2) => p2.contains(q2))) return true;
      if (p2.points.some((q2) => p1.contains(q2))) return true;
  ...
  var Rectangle = class _Rectangle {
    ...
    project(p) {
      ...
      for (const e of this.edges) {
        const q1 = e.project(p);
        if (!q2 || Point.distance(p, q1) < Point.distance(p, q2)) q2 = q1;
      }
      return q2;
    }
    at(t) {
    ...
    for (const match of path.match(pathSegment) || []) {
      ...
      for (const [i, p] of chunk(args, pathLength[uType]).entries()) {
        ...
        if (uType === "H") {
          ...
          points = chunk(p, 2).map((q2) => new Point(q2[0], q2[1]));
        }
  ...
  var SVGBaseView = class extends BaseView {
    ...
    setLine(p, q2) {
      this.setAttr("x1", p.x);
      this.setAttr("y1", p.y);
      this.setAttr("x2", q2.x);
      this.setAttr("y2", q2.y);
    }
  ...
  // content/quadratics/components/projectile.ts
  var Projectile = class extends CustomElementView {
    ready() {
  ...
  function intro4($step) {
    ...
    $system.isFinal = (expr) => {
      ...
      return expr.variables.length === 1 && expr.functions.includes("sup") && str.includes("89000") && str.includes("3250") && !str.includes("(");
    };
