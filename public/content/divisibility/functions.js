...
var StepFunctions = (() => {
  ...
  // content/divisibility/functions.ts
  var functions_exports = {};
  __export(functions_exports, {
    cicadas: () => cicadas,
    divisibility2: () => divisibility2,
    divisibility3a: () => divisibility3a,
    divisibility3b: () => divisibility3b,
    divisibility5: () => divisibility5,
    divisibility6: () => divisibility6,
    divisibility9: () => divisibility9,
    divisibilityGame: () => divisibilityGame,
  ...
  // content/shared/components/gameplay/gameplay.pug
  var gameplay_default = '<div class="dots"></div><div class="slide-template"><slot></slot></div><x-solved></x-solved>';

  // content/shared/components/gameplay/gameplay.ts
  var Gameplay = class extends CustomElementView {
    constructor() {
      super(...arguments);
      this.history = [];
    ...
    score(correct = false) {
      var _a2;
      (_a2 = this.$currentSlide) == null ? void 0 : _a2.addClass("done");
      ...
      if (!this.completed && this.history.filter((t) => t).length >= this.target[0]) {
        ...
        setTimeout(() => this.makeSlide(), 1e3);
      }
    }
    makeSlide(initial = false) {
      if (!this.slideGenerator) return;
      if (this.history.length >= this.target[1]) {
        this.history.shift();
        this.$dots.animate({ transform: ["none", "translateX(-16px)"] }, 300).promise.then(() => this.$dots.css("transform", "none"));
      ...
      if (this.$currentSlide) {
        ...
        $previous.animate({ transform: ["none", "translateX(-100%)"] }, 400).promise.then(() => $previous.remove());
      }
  ...
  Gameplay = __decorateClass([
    register("x-gameplay", { template: gameplay_default })
  ], Gameplay);
  ...
  function getText($step, id) {
    var _a2, _b;
    return ((_b = (_a2 = $step == null ? void 0 : $step.$course) == null ? void 0 : _a2.$tutor) == null ? void 0 : _b.hints[id]) || id;
  }
  function divisibilityGame($step) {
    const $gameplay = $step.$("x-gameplay");
    $gameplay.setSlideGenerator(($el, success, error) => {
      const answer = random_exports.smart(3, "factor-quiz");
      const n1 = random_exports.smart(9, "factor-quiz-number") + 2;
      const n2 = random_exports.smart(9, "factor-quiz-number") + 2;
      const val = [n1, n1];
      if (answer === 0) {
        val[1] = n1 * n2;
      } else if (answer === 1) {
        ...
        val[1] = n1 * n2 + random_exports.find([1, -1]);
        if (random_exports.bernoulli()) val.reverse();
      }
  ...
  function divisibility2($section) {
    numberGrid($section.$(".number-grid"), 1e3, "blue", (i) => i % 2 === 0);
  }
  function divisibility5($section) {
    numberGrid($section.$(".number-grid"), 1e3, "green", (i) => i % 5 === 0);
  }
  ...
  function divisibility3b($section) {
    numberGrid($section.$(".number-grid"), 1e3, "red", (i) => i % 3 === 0);
    setTimeout(() => {
  ...
  function factors2($section) {
    ...
    $slides.on("next", (x) => __async(this, null, function* () {
      if (x === 1) {
        $numbers[1].enter();
        $numbers[6].enter();
        $pairs[1].enter();
    ...
    $slides.on("back", (x) => __async(this, null, function* () {
      if (x === 0) {
        $numbers[1].exit();
        $numbers[6].exit();
        $pairs[1].exit();
  ...
  function primeTest($section) {
    ...
    $input.on("blur", () => {
      ...
      thread("/content/divisibility/worker.js", ["isPrime", v]).then((result) => $section.model.result = getText($section, result ? "is-prime" : "not-prime")).catch(() => $section.model.result = getText($section, "no-solution"));
    });
  }
  function primeGenerator($section) {
    $section.$(".btn").on("click", () => {
      ...
      $section.model.result = '<span class="loading"></span>';
      $section.score("calculator");
      thread("/content/divisibility/worker.js", ["getPrime", d], 1e4).then((result) => $section.model.result = numberFormat(result)).catch(() => $section.model.result = `Couldn\u2018t find a prime :(`);
    });
  }
  function ulam($section) {
    ...
    setTimeout(() => {
      ...
      $cells.forEach(($c) => {
        ...
        setTimeout(() => $c.addClass("red"), delay2 * 80);
        delay2 += 1;
      });
    }, 3e3);
  }
  ...
  function goldbach1($section) {
    ...
    $input.on("blur", () => {
      ...
      $section.model.result = '<span class="loading"></span>';
      $section.score("calculator");
      thread("/content/divisibility/worker.js", ["goldbach", v], 1e4).then((result) => $section.model.result = `${v} = ${result[0]} + ${result[1]}`).catch(() => $section.model.result = getText($section, "no-solution"));
    });
  }
  function riemann($section) {
    ...
    $log.points = list(3, 1e3).map((i) => ({ x: (i - 1) * dx, y: y0 - dy * i / Math.log(i) }));
