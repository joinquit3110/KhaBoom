...
var StepFunctions = (() => {
  ...
  // content/statistics/functions.ts
  ...
  function roulette($step) {
    ...
    rotateDisk($target, {
      ...
      draw(angle, isMomentum, dt) {
        for (const $w of $wheels) $w.setTransform(void 0, angle);
        if (!isMomentum) return;
        ballSpeed *= 0.985;
        if (Math.abs(ballSpeed) > 32e-5) {
  ...
  var generatePossibilitiesCached = cache(generatePossibilities);
  ...
  function compute(str) {
    ...
    for (let i = 2; i <= max; ++i) {
      ...
      const sum2 = total(observed);
      const chi = total(observed.map((o) => (o - sum2 / count) ** 2 / sum2 * count));
      const deg = count - 1;
      result = Math.min(result, random_exports.chiCDF(chi, deg));
    }
