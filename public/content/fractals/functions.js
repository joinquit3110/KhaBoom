...
var StepFunctions = (() => {
  ...
  __export(functions_exports, {
    ...
    julia2: () => julia2,
    koch: () => koch,
    mandelOrbits: () => mandelOrbits,
    mandelPaint: () => mandelPaint,
    mandelZoom: () => mandelZoom,
    pascal: () => pascal,
    pascalLarge: () => pascalLarge,
  ...
  // content/fractals/components/fractals.ts
  ...
  function drawKoch(last2, i) {
    if (i <= 0) return last2;
    const points = [];
    for (const e of last2.edges) {
  ...
  function koch($step) {
    const triangle2 = Polygon.regular(3, 150).shift(150, 150);
    $step.model.koch = drawKoch.bind(void 0, triangle2);
    const $slider = $step.$("x-slider");
  ...
  function pascalLarge($step) {
    ...
    const triangle2 = Polygon.regular(3, 546).shift(480, 560);
    const edge = triangle2.edges[1];
    const rows = 128;
    ...
    $step.model.watch((s) => {
      ...
      for (let n = 0; n < rows; ++n) {
        for (let k = 0; k <= n; ++k) {
          ...
          const fill2 = s.n <= 1 || cells[n][k] ? "#ddd" : color;
          $canvas.draw(new Circle(p, 3.2), { fill: fill2 });
        }
      }
    });
  }
  function chaosGame($step) {
    return __async(this, null, function* () {
      ...
      yield wait(2e3);
      const m1 = $geopad.drawPath(
        `segment(${startPoint.name},x0)`,
        { name: "m1", animated: 1e3, classes: "thin" }
      );
      yield wait(1e3);
      const p1 = $geopad.drawPoint(
        `m1.midpoint`,
      ...
      yield wait(2e3);
      m1.$el.exit("fade");
      const m2 = $geopad.drawPath(
        `segment(p1,x1)`,
        { name: "m2", animated: 1e3, classes: "thin" }
      );
      yield wait(1e3);
      const p2 = $geopad.drawPoint(
        `m2.midpoint`,
