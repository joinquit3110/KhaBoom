...
var StepFunctions = (() => {
  ...
  // content/linear-functions/functions.ts
  ...
  function linear(data, throughOrigin = false) {
    ...
    const intercept2 = sY / len - gradient * sX / len;
    return [intercept2, gradient];
  }
  ...
  function intercept($step) {
    ...
    const $lines = arrows.map(
      () => $N("line", { class: "arrow" }, $chart.$overlay)
    );
