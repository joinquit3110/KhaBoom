...
var StepFunctions = (() => {
  ...
  // content/complex/functions.ts
  var functions_exports = {};
  __export(functions_exports, {
    eulerApprox: () => eulerApprox,
  ...
  function complexPower(p, x) {
    const c = new Complex(p.x, p.y);
    const mod2 = Math.pow(c.modulus, x);
    const arg = c.argument * x;
    return new Point(mod2 * Math.cos(arg), mod2 * Math.sin(arg));
  }
  function complexRoot(p, n, i) {
    const c = new Complex(p.x, p.y);
