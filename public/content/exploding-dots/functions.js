...
var StepFunctions = (() => {
  ...
  // content/exploding-dots/functions.ts
  ...
  // content/exploding-dots/components/dot-machine.ts
  var enterAudio = new AudioPlayer("/audio/appear.mp3");
  var explodeAudio = new AudioPlayer("/audio/whoosh.mp3");
  var annihilateAudio = new AudioPlayer("/audio/disappear.mp3");
  var Cell = class extends EventTarget {
    constructor($dotMachine, initial = 0, index = 0) {
      ...
      const order = numberFormat(index > 0 ? Math.pow($dotMachine.base, index) : 1 / Math.pow($dotMachine.base, -index));
      $N("div", { class: "cell-order", html: order }, this.$el);
      if (initial) {
        this.rearrange(initial);
        for (let i = 0; i < initial; ++i) this.addDot(void 0, { count: false });
      }
    }
    get $fullDots() {
    ...
    addDot(posn, { className = "", dx = 0, audio = false, count = true } = {}) {
      ...
      $dot.animate({
        transform: [
          `translate(${posn.x}px, ${posn.y}px) scale(0.1)`,
    ...
    explode(recursive = false) {
      ...
      const target = next ? next.getDotPosition(next.$dots.length) : void 0;
      const transform = next ? target.add(next.$el.topLeftPosition).subtract(this.$el.topLeftPosition) : new Point(-54, 50);
      for (const $r of $remove) {
        $r.animate({ transform: `translate(${transform.x}px,${transform.y}px) scale(2)` }, 400, 400).promise.then(() => $r.remove());
      }
    ...
    annihilate() {
      ...
      for (let i = 0; i < n; ++i) {
        setTimeout(() => {
          ...
          $fullDots[i].animate({ transform: `translate(50px, 50px) scale(2)` }, 400, 400).promise.then(() => $fullDots[i].exit("pop"));
          $antiDots[i].addClass("warning");
          $antiDots[i].animate({ transform: `translate(50px, 50px) scale(2)` }, 400, 400).promise.then(() => $antiDots[i].exit("pop"));
        }, i * 600);
        setTimeout(() => annihilateAudio.play(), i * 600 + 500);
      }
    }
  };
  var DotMachine = class extends CustomElementView {
    constructor() {
      super(...arguments);
      this.cells = [];
    }
  ...
  function dots1($step) {
    ...
    $machine.on("add", ({ i, cell, point }) => {
      ...
      wait(1e3).then(() => cell.explode(true)).then(() => $step.score("dot"));
    });
  }
  function svgs($step) {
    let t = 1200;
  ...
  function dots2($step) {
    ...
    $machine.on("add", ({ i, cell, point }) => {
      ...
      cell.addDot(point, { audio: true });
      if (added === 2) {
        wait(1e3).then(() => cell.explode(true)).then(() => $step.score("dots"));
      }
    });
  }
  function addDots($machine, className) {
    for (let i = 0; i <= 4; ++i) {
      for (let x = 0; x < 6; ++x) {
        const $dot2 = $machine.cells[i].addDot();
        $dot2.addClass(className);
      }
    }
    const $dot = $machine.cells[4].addDot(void 0, { audio: true });
  ...
  function dots3($step) {
    ...
    $button.one("click", () => {
      ...
      setTimeout(() => addDots($machine, "light"), 1e3);
      setTimeout(() => $machine.explode().then(() => $step.score("dots")), 2e3);
    });
  }
  function factors(n) {
  ...
  function square2($step) {
    const $svg = $step.$("svg.square");
    const width = 320;
    $step.model.watch((state) => {
      $svg.removeChildren();
      const f = factors(state.x);
      const colours2 = random_exports.shuffle(Color.rainbow(2 * state.x));
      let c = 0;
      for (let i = 0; i < f[0]; ++i) {
        for (let j = 0; j < f[1]; ++j) {
          ...
          const odd = (i + j) % 2;
          $N("polygon", { points: `${r[0].x} ${r[0].y},${r[1].x} ${r[1].y},${r[odd ? 2 : 3].x} ${r[odd ? 2 : 3].y}`, fill: colours2[c] }, $svg);
          $N("polygon", { points: `${r[odd ? 0 : 1].x} ${r[odd ? 0 : 1].y},${r[2].x} ${r[2].y},${r[3].x} ${r[3].y}`, fill: colours2[c + 1] }, $svg);
          c += 2;
        }
      }
    });
