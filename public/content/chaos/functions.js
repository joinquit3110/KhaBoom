...
var StepFunctions = (() => {
  ...
  function pendulum($step) {
    ...
    $play.on("play", () => {
      ...
      setTimeout(() => $step.score("play"), 8e3);
    });
    $play.on("pause", () => {
      sim.pause();
      $step.score("play");
  ...
  function doublePendulum($step) {
    ...
    let showAll = false;
    const pendulums = [
      new DoublePendulum(state, ["a1", "a2"], center),
    ...
    $play.on("play", () => {
      setTimeout(() => $step.score("play1"), 12e3);
      if (showAll) setTimeout(() => $step.score("play2"), 8e3);
      for (const p of pendulums) p.set(model.d1, model.d2);
      pendulums[1].angles[1] += 1e-4;
      pendulums[2].angles[1] += 2e-4;
      pendulums[3].angles[1] += 3e-4;
      sim.play((dt) => {
  ...
  function butterfly1($step) {
    ...
    $butterfly.on("pointerdown", () => {
      ...
      setTimeout(() => $tornado.addClass("active"), 6e3);
      setTimeout(() => $tornado.removeClass("active"), 1e4);
      setTimeout(() => $step.score("flap"), 7e3);
    });
  }
  ...
  var butterfly2 = videoPlay;
  var dominoes = videoEnd;
  var applications = videoPlay;
  var popCulture = videoPlay;
  var RADIUS = 10;
  var Ball = class {
    constructor(x, y, fill2) {
      this.p = new Point(x, y);
      this.v = new Point(0, 0);
      this.$el = $N("circle", { r: RADIUS, cx: x, cy: y, fill: fill2 });
    }
  };
  ...
  function pool($step) {
    ...
    const $path = $N(
      "path",
      { fill: "transparent", stroke: "white", d: "M60,220" },
      $svg
    );
