...
var StepFunctions = (() => {
  ...
  // content/transformations/functions.ts
  var functions_exports = {};
  __export(functions_exports, {
    addSymmetries: () => addSymmetries,
    ...
    planets: () => planets,
    reflectionalSymmetry1: () => reflectionalSymmetry1,
    reflectionalSymmetry2: () => reflectionalSymmetry2,
    reflections: () => reflections,
    reflections1: () => reflections1,
    rigid1: () => rigid1,
    rotationalSymmetry2: () => rotationalSymmetry2,
    rotations: () => rotations,
    symmetry: () => symmetry,
    symmetryArithmetic: () => symmetryArithmetic,
    transformations: () => transformations,
    translations1: () => translations1,
    wallpaperGroups1: () => wallpaperGroups1,
    wallpaperGroups3: () => wallpaperGroups3
  });
  ...
  var Polygon = class _Polygon {
    ...
    reflect(line2) {
      const points = this.points.map((p) => p.reflect(line2));
      return new this.constructor(...points);
    }
    scale(sx, sy = sx) {
  ...
  function drawSVG(obj, options = {}) {
    ...
    if (isSegment(obj)) {
      ...
      let line2 = drawPath(obj.p1, obj.p2);
      if (options.mark) line2 += drawLineMark(obj, options.mark);
      if (options.arrows) line2 += drawLineArrows(obj, options.arrows);
      return line2;
    }
    if (isRay(obj)) {
      if (!options.box) return "";
    ...
    if (isLine(obj)) {
      ...
      let line2 = drawPath(points[0], points[1]);
      if (options.mark) line2 += drawLineMark(obj, options.mark);
      return line2;
    }
    if (isCircle(obj)) {
  ...
  // content/transformations/components/wallpaper.pug
  var wallpaper_default = '<x-select class="tabs"><div class="group" data-value="P1">1</div><div class="group" data-value="P2">2</div><div class="group" data-value="P3">3</div><div class="group" data-value="P4">4</div><div class="group" data-value="P6">5</div><div class="group" data-value="PM">6</div><div class="group" data-value="PMM">7</div><div class="group" data-value="P4M">8</div><div class="group" data-value="P6M">9</div><div class="group" data-value="P3M1">10</div><div class="group" data-value="P31M">11</div><div class="group" data-value="P4G">12</div><div class="group" data-value="CMM">13</div><div class="group" data-value="PMG">14</div><div class="group" data-value="PG">15</div><div class="group" data-value="CM">16</div><div class="group" data-value="PGG">17</div></x-select><canvas width="1920" height="1280"></canvas><x-select class="colours"><div class="colour"></div><div class="colour"></div><div class="colour"></div><div class="colour"></div><div class="colour"></div><div class="colour"></div><div class="colour"></div><div class="colour"></div></x-select><div class="btn-row"><button class="icon-btn save" title="Download image"><x-icon name="download"></x-icon></button><button class="icon-btn clear" title="Clear canvas"><x-icon name="restart"></x-icon></button></div>';

  // content/transformations/components/wallpaper.ts
  var width = 1920;
  var height = 1280;
  var pointX1M = new Point(240, 320);
  var pointX1Y1 = new Point(480, 320);
  var lineX1 = new Line(new Point(0, 320), new Point(480, 320));
  var lineY1 = new Line(new Point(480, 0), new Point(480, 320));
  var lineY2 = new Line(new Point(640, 0), new Point(640, 320));
  var pointS = new Point(360, 360);
  var lineS = new Line(new Point(0, 0), new Point(360, 360));
  var lineSI = new Line(new Point(0, 720), new Point(720, 0));
  ...
  function applyTransforms(point, [x, y], transforms) {
    ...
    for (const t of transforms) {
      for (const p of points.map((p2) => t(p2))) {
  ...
  var TRANSFORMATIONS = {
    ...
    P3: (p) => {
      ...
      return applyTransforms(p, [6 * w, h], [
        (p2) => p2.rotate(2 * Math.PI / 3, new Point(w, 0)),
        (p2) => p2.rotate(-2 * Math.PI / 3, new Point(w, h)),
        (p2) => p2.rotate(-2 * Math.PI / 3, new Point(5 * w, 0)),
        (p2) => p2.rotate(2 * Math.PI / 3, new Point(5 * w, h)),
        (p2) => p2.rotate(2 * Math.PI / 3, new Point(2 * w, h / 2)),
        (p2) => p2.rotate(-2 * Math.PI / 3, new Point(2 * w, h / 2)),
        (p2) => p2.rotate(2 * Math.PI / 3, new Point(4 * w, h / 2)),
        (p2) => p2.rotate(-2 * Math.PI / 3, new Point(4 * w, h / 2))
      ]);
    },
    P4: (p) => {
    ...
    P6: (p) => {
      ...
      return applyTransforms(p, [w, 2 * h], [
        (p2) => p2.rotate(Math.PI, new Point(w / 2, h)),
        (p2) => p2.rotate(Math.PI, new Point(w / 4, h / 2)),
        (p2) => p2.rotate(Math.PI, new Point(w / 4 * 3, h / 2)),
        (p2) => p2.rotate(2 * Math.PI / 3, new Point(0, h / 3 * 2)),
        (p2) => p2.rotate(-2 * Math.PI / 3, new Point(w, h / 3 * 2)),
        (p2) => p2.rotate(2 * Math.PI / 3, new Point(w / 2, h / 3)),
        (p2) => p2.rotate(-2 * Math.PI / 3, new Point(w / 2, h / 3)),
        (p2) => p2.rotate(Math.PI, new Point(w / 2, h))
      ]);
    },
    ...
    PMM: (p) => {
      ...
      const p2 = p1.reflect(lineY1);
      const p34 = [p1, p2].map((p3) => p3.reflect(lineX1));
    ...
    P4M: (p) => {
      ...
      const p3 = p2.rotate(Math.PI / 2, pointS);
      const p4 = p3.rotate(Math.PI / 2, pointS);
      const p5678 = [p1, p2, p3, p4].map((p5) => p5.reflect(lineS));
    ...
    P6M: (p) => {
      ...
      return applyTransforms(p, [w * 6, h * 2], [
        (p2) => p2.reflect(lineY(w * 3)),
        (p2) => p2.reflect(lineX(h)),
        (p2) => p2.reflect(line(0, 0, w, h)),
        (p2) => p2.reflect(line(w * 2, 0, w * 3, h)),
        (p2) => p2.reflect(line(0, 0, w * 3, h)),
        (p2) => p2.reflect(line(w, h, w * 2, 0)),
        (p2) => p2.reflect(lineY(w * 3)),
        (p2) => p2.reflect(lineX(h))
      ]);
    },
    P3M1: (p) => {
      ...
      return applyTransforms(p, [w * 3, h * 2], [
        (p2) => p2.reflect(lineX(h)),
        (p2) => p2.rotate(Math.PI * 2 / 3, new Point(1.5 * w, h)),
        (p2) => p2.rotate(-Math.PI * 2 / 3, new Point(1.5 * w, h)),
        (p2) => p2.reflect(line(0, 0, w, 2 * h)),
        (p2) => p2.reflect(line(w, 0, 2 * w, 2 * h)),
        (p2) => p2.reflect(line(2 * w, 0, 3 * w, 2 * h)),
        (p2) => p2.reflect(line(w, 0, 0, 2 * h)),
        (p2) => p2.reflect(line(2 * w, 0, w, 2 * h)),
        (p2) => p2.reflect(line(3 * w, 0, 2 * w, 2 * h))
      ]);
    },
    P31M: (p) => {
      return applyTransforms(p, [740, 2 * 640], [
        (p2) => p2.reflect(lineX(640)),
        (p2) => p2.rotate(2 * Math.PI / 3, new Point(0, 640 / 3 * 2)),
        (p2) => p2.rotate(-2 * Math.PI / 3, new Point(740, 640 / 3 * 2)),
        (p2) => p2.rotate(2 * Math.PI / 3, new Point(740 / 2, 640 / 3)),
        (p2) => p2.rotate(-2 * Math.PI / 3, new Point(740 / 2, 640 / 3)),
        (p2) => p2.reflect(line(0, 0, 740 / 2, 640)),
        (p2) => p2.reflect(line(740, 0, 740 / 2, 640)),
        (p2) => p2.reflect(lineX(640))
      ]);
    },
    P4G: (p) => {
      ...
      const p5678 = [p1, p2, p3, p4].map((p5) => p5.reflect(lineSI));
      return grid([p1, p2, p3, p4, ...p5678], 720, 720);
    },
    CMM: (p) => {
      ...
      const p34 = [p1, p2].map((p3) => p3.reflect(lineY2));
      const p5678 = [p1, p2, ...p34].map((p3) => p3.reflect(lineX1));
    ...
    PGG: (p) => {
      ...
      const p34 = [p1, p2].map((p3) => p3.rotate(Math.PI, pointX1Y1));
      return grid([p1, p2, ...p34], 960, 640);
    }
  };
  ...
  var Wallpaper = class extends CustomElementView {
    ready() {
  ...
  Wallpaper = __decorateClass([
    register("x-wallpaper", { template: wallpaper_default })
  ], Wallpaper);
  ...
  function play($step, $el, duration, score, callback) {
    ...
    $play.on("play", () => {
      ...
      setTimeout(() => $play.reset(), duration + 2e3);
    });
  }
  function expandSegment($geopad, [e1, e2], line2) {
    const swap = Point.distance(e1.value, line2.p1) > Point.distance(e2.value, line2.p1);
    $geopad.animatePoint(swap ? e2.name : e1.name, line2.p1);
    $geopad.animatePoint(swap ? e1.name : e2.name, line2.p2);
  }
  ...
  function transformations($step) {
    ...
    play($step, $animations[0], 1e3, "t1", () => {
      $images[0].animate(
        { transform: ["none", "translate(95px,45px) rotate(18deg) scale(1.2)"] },
        1e3
      );
    });
    play($step, $animations[1], 1e3, "t2", () => {
      $images[1].animate({
        transform: ["none", "translate(80px,100px) scale(0.9,1.3) rotate(-30deg)"]
      }, 1e3);
    });
    play($step, $animations[2], 1e3, "t3", () => {
      $images[2].animate({
        transform: [
          "translate(-20px,40px) rotate(-30deg)",
          "rotate(30deg) translate(135px,0) scale(0.8)"
        ],
        rx: ["50px", 0],
        ry: ["50px", 0]
      }, 1e3);
  ...
  function rigid1($step) {
    ...
    play($step, $animations[0], 1e3, "t1", () => {
      $arrow.hide();
      $images[0].animate(
        { transform: ["none", "translate(90px,-40px)"] },
        1e3,
        0,
        "linear"
      );
      $lines[0].enter("draw", 1e3);
      $arrow.enter("fade", 400, 900);
    });
    play($step, $animations[1], 2e3, "t2", () => {
      $images[1].css("transform", "none");
      $lines[1].enter("draw", 1e3);
      $images[1].animate(
    ...
    play($step, $animations[2], 2e3, "t3", () => {
      $images[2].css("transform", "none");
      for (let i = 2; i < 7; ++i) $lines[i].enter("draw", 500);
      $images[2].animate({ transform: "rotate(84deg)" }, 1e3, 1e3);
    });
  }
  ...
  var reflections1 = drawThreeShapes;
  var rotations = drawThreeShapes;
  var reflectionalSymmetry2 = drawThreeShapes;
  var rotationalSymmetry2 = drawThreeShapes;
  ...
  function add(a, b) {
    if (a < 4 && b < 4) return (a + b) % 4;
    if (a >= 4 && b >= 4) return (a - b + 4) % 4;
    if (a > b) return 4 + (a - b) % 4;
    return 4 + (a + b) % 4;
  }
  function makeSquare(i, x, $parent, delay2 = 0) {
    const $el = $N("img", {
      src: `/content/transformations/images/cube-${i}.svg`,
      style: `left: ${8 + x * 70}px;`
    }, $parent);
    $el.enter("pop", 200, delay2);
    return $el;
  }
