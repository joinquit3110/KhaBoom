...
var StepFunctions = (() => {
  ...
  // content/matrices/functions.ts
  var functions_exports = {};
  __export(functions_exports, {
    basicTransformations: () => basicTransformations,
    determinants: () => determinants,
    mathigonMatrix: () => mathigonMatrix,
    playWithMe: () => playWithMe,
    rocket: () => rocket,
    transformsCalculator: () => transformsCalculator,
  ...
  // content/matrices/functions.ts
  function rocket($step) {
    const $geo = $step.$("x-geopad");
    $step.model.th = 0;
    $geo.on("click", () => __async(this, null, function* () {
  ...
  function generateTransformFunction(shape) {
    return function(a, b, c, d) {
      const pointString = shape.map((p) => [p[0] - 40, p[1] - 40]).map((p) => [[p[0]], [p[1]]]).map((p) => matrix_exports.product([
        [a, -b],
        [-c, d]
      ], p)).map((p) => [p[0][0], p[1][0]]).map((p) => [p[0] + 110, p[1] + 110]).map((point) => point.join(",")).join(" ");
      return `<polygon points="${pointString}" opacity="0.5" />`;
    };
  }
  function basicTransformations($step) {
    const SHAPE = [[30, 10], [10, 70], [60, 70], [50, 10]];
    $step.model.polygonTransform = generateTransformFunction(SHAPE);
  }
  function mathigonMatrix($step) {
    const SHAPES = {
    ...
    $step.model.polygonTransform = (a, b, c, d, color) => {
      return generateTransformFunction(SHAPES[color])(a, b, c, d);
    };
  }
  function playWithMe($step) {
  ...
  function parseNumberArray(str) {
    ...
    return result.slice(1, 3).map((n) => Number.parseFloat(n));
  }
  function animateTransformationOnGeo(geo, iv, jv, m, time) {
    geo.animatePoint(iv, new Point(m[0][0], m[0][1]), time);
    geo.animatePoint(jv, new Point(m[1][0], m[1][1]), time);
  }
  function transformsCalculator($step) {
    var _a2;
    const $cubes = $step.$$(".cube");
    const $geopads = (_a2 = $step.$(".display")) == null ? void 0 : _a2.$$("x-geopad");
    const $calc = $step.$(".calc");
    const $clear = $step.$(".clear");
    let a = void 0;
    let b = void 0;
    let c = void 0;
    const I = matrix_exports.identity(2);
    function drawUnitVectorsToGeo(geo, matrix, name) {
      geo.drawPoint(
        "point(0,0)",
        { name: "origin", interactive: false, classes: "hidden" }
      );
      geo.drawPoint(
        `point(${matrix[0][0]},${matrix[0][1]})`,
        { name: `i${name}`, interactive: false, classes: "hidden" }
      );
      geo.drawPoint(
        `point(${matrix[1][0]},${matrix[1][1]})`,
    ...
    for (const [_i, $c] of $cubes.entries()) {
      $c.on("click", () => {
        if (a === void 0) {
          ...
          drawUnitVectorsToGeo($geopads[0], I, "a");
          ...
          drawUnitVectorsToGeo($geopads[1], I, "b");
