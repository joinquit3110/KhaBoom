...
var StepFunctions = (() => {
  ...
  __export(functions_exports, {
    ...
    julia2: () => julia2,
    mandelPaint: () => mandelPaint,
    mandelZoom: () => mandelZoom,
    maps1: () => maps1,
    mapsV2: () => maps1,
    planarity: () => planarity,
    salesman4: () => salesman4,
    threeBodies: () => threeBodies,
    utilities: () => utilities,
    utilitiesV2: () => utilities
  });
  ...
  function list(a, b, step2 = 1) {
    const arr = [];
    if (b === void 0 && a >= 0) {
      for (let i = 0; i < a; i += step2) arr.push(i);
    } else if (b === void 0) {
      for (let i = 0; i > a; i -= step2) arr.push(i);
    } else if (a <= b) {
      for (let i = a; i <= b; i += step2) arr.push(i);
    } else {
      for (let i = a; i >= b; i -= step2) arr.push(i);
    }
    return arr;
  ...
  var Color = class _Color {
    ...
    /** Generates a rainbow gradient with a given number of steps. */
    static gradient(colors2, steps) {
      return tabulate((x) => getColorAt(colors2, x / (steps - 1)), steps);
    }
    ...
    static mixMany(colors2, weights) {
      if (!weights) weights = colors2.map(() => 1);
      const weight = total(weights);
      const hsl = colors2.map((c) => c.hsl);
      const hue = hsl.map((c) => c[0]);
      const altHue = hue.map((h3) => h3 < 180 ? h3 + 360 : h3);
      const hueWeights = weights.map((w, i) => w * Math.sqrt(colors2[i].chroma));
  ...
  var Vector = class _Vector extends Array {
    ...
    static dot(v1, v2) {
      return total(_Vector.product(v1, v2));
    }
  ...
  var SVGBaseView = class extends BaseView {
    ...
    setRect(rect2) {
      this.setAttr("x", rect2.p.x);
      this.setAttr("y", rect2.p.y);
      this.setAttr("width", rect2.w);
      this.setAttr("height", rect2.h);
    }
  ...
  // content/shared/components/solved/solved.pug
  var solved_default = '<button class="close"><x-icon name="close"></x-icon></button><x-icon class="sketch" name="trophy" size="160"></x-icon><div class="message">Well done!</div>';

  // content/shared/components/solved/solved.ts
  var colors = loop(["red", "orange", "lime", "green", "teal", "blue", "purple"]);
  var images = loop(["badge", "homework", "laurels", "lightbulb", "owl", "scroll", "trophy"]);
  var Solved = class extends CustomElementView {
    constructor() {
      super(...arguments);
      this.visible = false;
    }
    ...
    enter() {
      ...
      this.visible = true;
      const $step = this.parents("x-step")[0];
      const hint = ((_a2 = $step == null ? void 0 : $step.addHint("correct")) == null ? void 0 : _a2.text) || "Well done!";
  ...
  Solved = __decorateClass([
    register("x-solved", { template: solved_default })
  ], Solved);

  // content/graph-theory/components/graph.ts
  ...
  var Graph = class extends EventTarget {
    ...
    load(vertices, edges, posn) {
      ...
      this.vertices = list(vertices).map((v) => {
        ...
        const $el = this.options.icon ? $N("path", { class: "node", d: this.options.icon }, this.$vertices) : $N("circle", { class: "node", r: this.options.r || 5 }, this.$vertices);
        if (this.options.vertex) $el.css("fill", run(this.options.vertex, v));
        return { $el, posn: { x, y }, neighbours: [], v: { x: 0, y: 0 } };
      });
    ...
    physics() {
      ...
      for (const [i, v] of this.vertices.entries()) {
        ...
        for (const u of this.vertices) {
          ...
          if (nearlyEquals(d, 0, 1e-3)) d = 1e-3;
        ...
        positions2[i] = { x: v.posn.x + v.v.x, y: v.posn.y + v.v.y };
      }
      this.stable = totalMoved < 0.5 && !this.dragging;
      this.arrange(positions2);
    }
    arrange(positions2 = []) {
      let center = void 0;
      for (const [i, v] of this.vertices.entries()) {
        v.posn = positions2[i] || v.posn;
      ...
      for (const e of this.edges) {
        if (e.vertices[0] === e.vertices[1]) {
          if (!center) center = Point.average(...this.vertices.map((v2) => v2.posn));
  ...
  // content/graph-theory/components/sketch.ts
  var Sketch = class extends EventTarget {
  ...
  // content/graph-theory/components/four-colour-maps.ts
  var borders = [
    {
      AK: ["WA"],
      WA: ["AK", "ID", "OR"],
    ...
    },
    {
      ...
      Brandenburg: ["Mecklenburg-Vorpommern", "Niedersachsen", "Sachsen-Anhalt", "Sachsen", "Berlin"],
      Berlin: ["Brandenburg"],
      "Mecklenburg-Vorpommern": ["Schleswig-Holstein", "Niedersachsen", "Brandenburg"],
      Niedersachsen: ["Schleswig-Holstein", "Hamburg", "Mecklenburg-Vorpommern", "Brandenburg", "Sachsen-Anhalt", "Th\xFCringen", "Hessen", "Nordrhein-Westfalen", "Bremen"],
      Bremen: ["Niedersachsen"],
      "Sachsen-Anhalt": ["Niedersachsen", "Brandenburg", "Sachsen", "Th\xFCringen"],
      Sachsen: ["Brandenburg", "Sachsen-Anhalt", "Th\xFCringen", "Bayern"],
      Th\u00FCringen: ["Hessen", "Niedersachsen", "Sachsen-Anhalt", "Sachsen", "Bayern"],
      "Nordrhein-Westfalen": ["Niedersachsen", "Hessen", "Rheinland-Pfalz"],
      Hessen: ["Rheinland-Pfalz", "Nordrhein-Westfalen", "Niedersachsen", "Th\xFCringen", "Bayern", "Baden_W\xFCrttemberg"],
      "Rheinland-Pfalz": ["Nordrhein-Westfalen", "Hessen", "Baden_W\xFCrttemberg", "Saarland"],
      Saarland: ["Rheinland-Pfalz"],
      Baden_W\u00FCrttemberg: ["Rheinland-Pfalz", "Hessen", "Bayern"],
      Bayern: ["Baden_W\xFCrttemberg", "Hessen", "Th\xFCringen", "Sachsen"]
    },
    {
      ...
      Bedfordshire: ["Northamptonshire", "Cambridgeshire", "Hertfordshire", "Buckinghamshire"],
      East_Sussex: ["West_Sussex", "Kent", "Surrey"],
  ...
  // content/graph-theory/components/geometry.ts
  ...
  // content/graph-theory/functions.ts
    ...
    $section.$$(".tab").forEach(($el, i) => {
      ...
      $bridges.forEach(($bridge) => {
        ...
        $bridge.on("pointerleave", (e) => {
          ...
          if (Point.distance(enter2 || out, out) < 40) {
            crossed = false;
          } else {
  ...
  function utilities($section) {
    ...
    const sectors = /* @__PURE__ */ new WeakMap();
    function clear() {
    ...
    for (const $ut of $section.$$(".utility")) {
      ...
      $ut.on("pointerdown", (e) => {
        ...
        for (const e2 of errors) {
          if (sectors.has(e2)) sectors.get(e2).css("opacity", 0);
        }
        errors = [];
        map.start(p);
        map.drawing = false;
      ...
      $ut.on("pointerenter", () => {
        ...
        if (startUtility.data.type === dataType) {
          ...
          const sector = startUtility.data.type === "house" ? $("." + $ut.data.utility, currentUtility) : $("." + currentUtility.data.utility, $ut);
  ...
  function planarity($section) {
    const $svg = $section.$("svg#planarity");
      ...
      for (let i = 0; i < edges.length; ++i) {
        ...
        for (let j = i + 1; j < edges.length; ++j) {
          ...
          if (intersections(e1, e2).length) {
            edges[i].intersect = edges[j].intersect = edges[i].vertices[0].intersect = edges[i].vertices[1].intersect = edges[j].vertices[0].intersect = edges[j].vertices[1].intersect = true;
            count += 1;
          }
        }
      }
    ...
    function generateGraph(n) {
      ...
      for (let i = 0; i < n; ++i) addEdge(i, random_exports.integer(n));
      for (let i = 0; i < n; ++i) for (let j = i + 1; j < n; ++j) addEdge(i, j);
      graph.load(n, edges, shuffle2(n));
      while (!intersect(graph.edges) && n > 4) {
        graph.arrange(shuffle2(n));
      }
      creating = false;
    }
    graph.on("update", () => {
  ...
  function maps1($section) {
    const colours3 = [RED, BLUE, GREEN, YELLOW, ORANGE, PURPLE, TEAL];
    const $colours = $section.$$(".four-colour-icon");
    $colours[0].addClass("on");
    let activeColour = 0;
    let warned = false;
    $colours.forEach(($el, i) => {
      $el.css("background", colours3[i]);
      $el.on("click", () => {
    ...
    $section.$$(".tab").forEach(($map, i) => {
      ...
      $countries.forEach(($c) => {
        ...
        $c.on("click", () => {
          ...
          if (countryColours[id] != void 0) --colourUses[countryColours[id]];
          ++colourUses[activeColour];
          $count.textStr = used = colourUses.filter((x) => x > 0).length;
          countryColours[id] = activeColour;
          $c.css("fill", colours3[activeColour]);
          if (used < completed && countryIds.every((id2) => countryColours[id2] != void 0)) {
            completed = used;
            if (used <= 4) {
              $solveds[i].enter();
              $section.score("map-" + i);
  ...
  // content/fractals/components/chaos-game.ts
  ...
  var ChaosGame = class {
    ...
    getNextIndex() {
      let index = random_exports.integer(this.vertices.length);
      if (this.rule === "repeat" && index === this.lastIndex) {
        ...
        index = random([
          this.lastIndex,
          (this.lastIndex + 1) % this.vertices.length,
  ...
  // content/fractals/components/menger-sponge.ts
  var positions = [];
  ...
  var MengerSponge = class extends Solid {
    created() {
      ...
      this.addMesh(() => {
        ...
        const material = new THREE.MeshLambertMaterial(
          { color: 16616448, flatShading: true }
        );
  ...
  MengerSponge = __decorateClass([
    register("x-menger-sponge")
  ], MengerSponge);

  // content/fractals/components/sierpinski-tetrahedra.ts
  var SierpinskiTetrahedra = class extends Solid {
    constructor() {
      super(...arguments);
  ...
  SierpinskiTetrahedra = __decorateClass([
    register("x-sierpinski-tetrahedra")
  ], SierpinskiTetrahedra);
  var SQRT = Math.sqrt(2);
  var SierpinskiPyramid = class extends SierpinskiTetrahedra {
    constructor() {
      super(...arguments);
      this.positions = [
        [0, 1, 0],
  ...
  SierpinskiPyramid = __decorateClass([
    register("x-sierpinski-pyramid")
  ], SierpinskiPyramid);

  // content/fractals/components/automata.ts
  var RULES = ["000", "001", "010", "100", "011", "101", "110", "111"];
  var MARGIN = 5;
  var CELL_SIZE = 12;
  var HIGHLIGHT = "M-15,-15l0,17l12,0l0,12l17,0l0,-12l11,0l0,-17Z";
  ...
  var CellularAutomaton = class extends CustomElementView {
    ready() {
      ...
      for (const [i, rule] of RULES.entries()) {
        ...
        const $highlight2 = $N("path", { d: HIGHLIGHT, class: "highlight" }, $svg);
        $highlight2.translate(MARGIN + CELL_SIZE, MARGIN + CELL_SIZE);
      ...
      pointerOver($grid, {
        ...
        move: (p) => {
          ...
          if (p.equals(highlightPosition)) return;
  ...
  CellularAutomaton = __decorateClass([
    register("x-automaton")
  ], CellularAutomaton);

  // content/fractals/functions.ts
  var colours2 = Color.gradient(["#22ab24", "#0f82f2"], 9).map((c) => c.toString());
  ...
  function julia2($step) {
    ...
    const resolution = pB.dx / vB.dx;
    const juliaCanvas = new JuliaCanvas(pB, vB, $canvas, resolution);
    $step.model.watch((m) => __async(this, null, function* () {
      return juliaCanvas.draw(m.c);
    }));
  }
  ...
  // content/chaos/components/double-pendulum.ts
  var PI2 = Math.PI / 2;

  // content/chaos/components/pool-table.ts
  var PoolTable = class extends CustomElementView {
    ready() {
      const $svg = this.$("svg");
  ...
  PoolTable = __decorateClass([
    register("x-pool-table")
  ], PoolTable);

  // content/chaos/components/water-ripples.ts
  var THRESHOLD = 0.05;
  var TIMEOUT;
  var WaterCanvas = class extends CustomElementView {
    constructor() {
      super(...arguments);
      this.isAnimating = false;
      // Amount of color highlighting.
      this.touchSize = 10;
    }
    ready() {
    ...
    startAnimation() {
      ...
      TIMEOUT = window.setTimeout(() => this.damping = 0.95, 1e4);
      this.damping = 0.994;
    ...
    updateCanvas() {
      ...
      for (let i = 0; i < pixelsOut.length; i += 4) {
        ...
        const reflection2 = 1 + strength * this.reflection;
        const x1 = clamp(x + refraction, 0, this.rx - 1);
        const y1 = clamp(y + refraction, 0, this.ry - 1);
        const i1 = (y1 * this.rx + x1) * 4;
        pixelsOut[i] = this.imagePixels[i1] * reflection2;
        pixelsOut[i + 1] = this.imagePixels[i1 + 1] * reflection2;
        pixelsOut[i + 2] = this.imagePixels[i1 + 2] * reflection2;
      }
    ...
    updateWater() {
      ...
      for (let x = 0; x < this.sx; x++) {
        for (let y = 0; y < this.sy; y++) {
          ...
          val = (val / 2 - this.depthMap2[x][y]) * this.damping;
          val = clamp(val, -this.clipping, this.clipping);
  ...
  WaterCanvas = __decorateClass([
    register("x-water-canvas")
  ], WaterCanvas);

  // content/chaos/functions.ts
  var collision = new AudioPlayer("/content/chaos/images/ball.mp3");
  function threeBodies($step) {
    ...
    function acceleration(i) {
      ...
      for (const j of ["a", "b", "c"]) {
        ...
        gravity = gravity.translate(dx.scale(16e5 / dx.length ** 3));
      }
      return gravity;
    }
    function step2(dt) {
    ...
    $play.on("play", () => sim.play(step2, 0.05, 250));
      ...
      slide($c, {
        ...
        move: (p, start) => {
          ...
          if (snapCircles.length === 1) {
            ...
            for (const p2 of intersections(snapCircles[0], snapCircles[1])) {
              if (Point.distance(p2, this.posn) < 3) this.posn = p2;
            }
          }
          $c.setCenter(this.posn);
          $t.setCenter(this.posn.shift(0, 3.4));
