...
var StepFunctions = (() => {
  ...
  // content/euclidean-geometry/functions.ts
  var functions_exports = {};
  __export(functions_exports, {
    congruence: () => congruence,
    crane: () => crane,
    equilateral: () => equilateral,
    origamiApplications: () => origamiApplications,
    origamiApplications3: () => origamiApplications3,
    origamiApplications4: () => origamiApplications4,
    origamiDna: () => origamiDna,
    origamiWings: () => origamiWings,
    thales: () => thales,
    tools: () => tools
  });
  ...
  function thales($step) {
    ...
    $geopad.on("add:point", ({ point }) => {
      if (!a) {
        ...
        $geopad.drawPath(
          `arc(line(${a},${b}).midpoint,${a},pi)`,
          { name: "semicirc", animated: 2e3, target: "circumf" }
        );
        $step.score("p2");
      } else if (!c) {
        c = point.name;
        ...
        $geopad.drawPath(
          (m) => m.angle(m[b], m[c], m[a]),
          { animated: 500, target: "angle", classes: "yellow thin" }
        );
        $geopad.drawPath(
          (m) => m.triangle(m[a], m[c], m[b]),
          { animated: 2e3, target: "triangle", classes: "red" }
        );
        $step.score("p3");
  ...
  function congruence($step) {
    ...
    slide($svg, {
      ...
      move(posn) {
        ...
        if (!$line) return;
        const to = centers.findIndex((c2) => Point.distance(c2, posn) < 40);
        if (to < 0 || from === to) {
          $line.exit("draw");
          return from = $line = void 0;
        }
    ...
    $step.one(
      "score-pair-e1-e2 score-pair-e1-e3 score-pair-e2-e3",
  ...
  function tools($step) {
    ...
    $geopads.forEach(($g, i) => {
      ...
      $play.on("play", () => {
        $g.animateConstruction(names[i]).then(() => {
          $play.reset();
  ...
  function equilateral($step) {
    return __async(this, null, function* () {
      ...
      const s0 = yield $geopad.waitForPath(
        (p) => isLineLike(p),
        drawingOptions
      );
  ...
  var origamiApplications = scoreOnVideo;
  var origamiApplications3 = scoreOnVideo;
  var origamiApplications4 = scoreOnVideo;
  var origamiWings = scoreOnVideo;
  var origamiDna = scoreOnVideo;
