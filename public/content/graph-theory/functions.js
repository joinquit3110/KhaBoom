...
var StepFunctions = (() => {
  ...
  __export(functions_exports, {
    ...
    euler3: () => euler3,
    euler4: () => euler4,
    handshakes1: () => handshakes1,
    handshakes2: () => handshakes2,
    handshakes2a: () => handshakes2a,
    handshakes3: () => handshakes3,
    handshakes4: () => handshakes4,
    intro: () => intro,
    intro1: () => intro1,
    intro2: () => intro2,
    intro3: () => intro3,
  ...
  function intro2($section) {
    ...
    new Graph(
      ...
      ],
      {
        ...
        edge: (u, v) => u < 6 && v < 6 && !(u === 1 && v === 3) ? BLUE : "#DDD"
      }
    );
    new Graph(
      graphs[2],
      ...
      ],
      {
        ...
        edge: (u, v) => u < 5 && v < 5 && !(v === 3 && (u === 0 || u === 1)) ? GREEN : "#DDD"
      }
    );
    new Graph(
      graphs[4],
      ...
      ],
      {
        ...
        edge: (u, v) => u < 6 && v < 6 && !(u === 1 && (v === 3 || v === 4)) ? YELLOW : "#DDD"
      }
    );
  }
  ...
  function handshakes2($step) {
    $step.model.handshakeTable = (n) => {
      const colours2 = Color.rainbow($step.model.n);
      function makePerson(x, y) {
        ...
        return [
          ...
          '">',
          y + 1,
          '</span> + <span class="person" style="background: ',
          colours2[newX],
          '">',
          newX + 1,
          "</span></td>"
      ...
      return '<table class="handshakes grid">' + tabulate2D(makePerson, n - 1, n).map((x) => `<tr>${x.join("")}</tr>`).join("") + "</table>";
    };
  }
  ...
  function bridges1($section) {
    ...
    $section.onScore("blank-0 blank-1", () => {
      setTimeout(() => $trace.enter("draw", 4e3), 2e3);
    });
  }
  function bridges3($section) {
    ...
    $section.model.watch((s) => {
      ...
      for (const $c of $circles) {
        ...
        $c.css("fill", "" + colours2[s.colour][y - 2]);
      }
    });
  }
  function bridges4($section) {
    ...
    $trace.hide();
    $slideshow.on("next", (x) => __async(this, null, function* () {
      if (x < 4) {
        ...
        $trace.enter("draw", 5e3, 800);
      }
    }));
    $slideshow.on("back", (x) => __async(this, null, function* () {
      if (x < 3) {
  ...
  function utilities1($section) {
    ...
    $section.onScore("blank-0", () => transition2(k4, p4x));
    $section.onScore("blank-1", () => {
      transition2(k5, p5x);
  ...
  function euler2($section) {
    ...
    const positions = [
      ...
      // show 0, 1, 3
      [{ x: 270, y: 30 }, { x: 150, y: 100 }, { x: 270, y: 170 }, { x: 390, y: 100 }]
      // show all
    ];
    let slide2 = 0;
    $vertices.forEach(($v2, i) => $v2.setCenter(positions[0][i]));
    $edges.forEach(($e2, i) => {
      $e2.setLine(positions[0][i], positions[0][(i + 1) % 4]);
    });
    $slideshow.on("next back", (s) => {
      const start = positions[slide2];
      const end = positions[s];
      slide2 = s;
      animate((x) => {
        $vertices.forEach(($v2, i) => {
          $v2.setCenter(Point.interpolate(start[i], end[i], x));
        });
        $edges.forEach(($e2, i) => {
          $e2.setLine(
            Point.interpolate(start[i], end[i], x),
  ...
  function maps1($section) {
    const colours2 = [RED, BLUE, GREEN, YELLOW, ORANGE, PURPLE, TEAL];
    const $colours = $section.$$(".four-colour-icon");
    $colours[0].addClass("on");
    let activeColour = 0;
    let warned = false;
    $colours.forEach(($el, i) => {
      $el.css("background", colours2[i]);
      $el.on("click", () => {
    ...
    $section.$$(".tab").forEach(($map, i) => {
      ...
      $countries.forEach(($c) => {
        ...
        $c.on("click", () => {
          ...
          $c.css("fill", colours2[activeColour]);
  ...
  function salesman2($section) {
    ...
    $section.model.tsnPaths = (x) => {
      return list(x, 1).join(" \xD7 ") + " = " + numberFormat(factorial(x));
    };
  }
