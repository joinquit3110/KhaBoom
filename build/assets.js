...
async function bundleStyles(srcPath, destPath, minify = false, watch = false) {
  ...
  const banner = `/* ${CONFIG.banner}, generated by Kha-Boom! */\n`;
...
async function bundleScripts(srcPath, destPath, minify = false, watch = false, options = {}) {
  ...
  for (const locale of options.locales || ['en']) {
    const result = await esbuild.build({
      ...
      metafile: watch,
      write: false,
      banner: {js: `/* ${CONFIG.banner}, generated by Kha-Boom! */`}
    });
...
async function buildAssets(minify = false, watch = false, locales = ['en']) {
  ...
  for (const {src, dest} of getAssetFiles('*.ts')) {
    if (src.endsWith('.d.ts') || src.includes('course.ts')) continue;
  ...
  for (const {src, dest} of getAssetFiles('*.scss')) {
    if (src.includes('course.scss')) continue;
    promises.push(bundleStyles(src, dest, minify, watch).catch(error(src)));
  }

  // Copy custom course files from frontend folder
  try {
    const frontendCourseCSS = path.join(process.cwd(), 'frontend', 'course.css');
    const frontendCourseJS = path.join(process.cwd(), 'frontend', 'course.js');
    
    if (fs.existsSync(frontendCourseCSS)) {
      const cssContent = readFile(frontendCourseCSS);
      await writeFile(path.join(OUTPUT, 'course.css'), cssContent);
      success('frontend/course.css copied', 0);
    }
    
    if (fs.existsSync(frontendCourseJS)) {
      const jsContent = readFile(frontendCourseJS);
      await writeFile(path.join(OUTPUT, 'course.js'), jsContent);
      success('frontend/course.js copied', 0);
    }
  } catch (err) {
    error('custom course files')('Failed to copy course files: ' + err.message);
  }
