...
class AIService {
    ...
    loadCourseKnowledge() {
        const contentDir = (0, path_1.join)(process.cwd(), 'content');
        try {
            const courses = (0, fs_1.readdirSync)(contentDir);
            for (const courseId of courses) {
                const coursePath = (0, path_1.join)(contentDir, courseId);
                if (!(0, fs_1.statSync)(coursePath).isDirectory())
                    continue;
                let courseContent = '';
                // Read main content file
                const contentFile = (0, path_1.join)(coursePath, 'content.md');
                try {
                    courseContent += (0, fs_1.readFileSync)(contentFile, 'utf-8');
                }
                catch (e) {
                    console.warn(`Could not read content.md for course ${courseId}`);
                }
                // Read hints file
                const hintsFile = (0, path_1.join)(coursePath, 'hints.yaml');
                try {
                    courseContent += '\n\nHints:\n' + (0, fs_1.readFileSync)(hintsFile, 'utf-8');
                }
                catch (e) {
                    console.warn(`Could not read hints.yaml for course ${courseId}`);
                }
                // Read functions file
                const functionsFile = (0, path_1.join)(coursePath, 'functions.ts');
                try {
                    courseContent += '\n\nFunctions:\n' + (0, fs_1.readFileSync)(functionsFile, 'utf-8');
                }
                catch (e) {
    ...
    getChatSession(userId, courseId) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                let session = yield chat_session_1.ChatSession.getActiveSession(userId, courseId);
                if (!session) {
                    session = yield chat_session_1.ChatSession.createNewSession(userId, courseId);
                }
                return session;
            }
            catch (error) {
    ...
    createNewChat(userId, courseId) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const session = yield chat_session_1.ChatSession.createNewSession(userId, courseId);
    ...
    // Get chat history
    getChatHistory(userId, courseId) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const session = yield chat_session_1.ChatSession.getActiveSession(userId, courseId);
    ...
    // Delete chat session
    deleteChat(userId, courseId) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const result = yield chat_session_1.ChatSession.updateMany({ userId, courseId, isActive: true }, { isActive: false });
    ...
    generateResponse(query, user, course, progress) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a, _b;
            let session = yield this.getChatSession((user === null || user === void 0 ? void 0 : user.id) || 'anonymous', course.id);
            ...
            if (isFirstMessage && course.id === 'dashboard') {
                ...
                if (!session) {
                    const sessionId = yield this.createNewChat((user === null || user === void 0 ? void 0 : user.id) || 'anonymous', course.id);
                    session = yield this.getChatSession((user === null || user === void 0 ? void 0 : user.id) || 'anonymous', course.id);
                }
                if (session) {
...
Feel free to ask me any questions about the course material!`;
            ...
            if (session) {
                ...
                yield this.checkAndSummarizeChat(session);
            ...
            try {
                ...
                const response = ((_b = (_a = completion.choices[0]) === null || _a === void 0 ? void 0 : _a.message) === null || _b === void 0 ? void 0 : _b.content) || 'I apologize, but I cannot provide a response at this time.';
    ...
    isValidCourseUrl(url) {
        return Object.values(search_1.SEARCH_DOCS).some((doc) => doc.url === url);
    }
    // Get valid course links from SEARCH_DOCS instead of hardcoding
    getValidCourseLinks() {
        const validCourses = Object.values(search_1.SEARCH_DOCS)
            .filter((doc) => doc.type === 'course' && doc.url.startsWith('/course/'))
            .reduce((acc, doc) => {
        ...
        const links = Object.values(validCourses)
            .slice(0, 5)
            .map((doc) => `• <a href="${doc.url}" style="color: #667eea; text-decoration: none;">${doc.title}</a> - ${doc.subtitle}`)
            .join('<br>');
        return links;
    }
    // Filter out fake URLs from response
    filterFakeLinks(response) {
    ...
    buildSystemPrompt(course, user, progress) {
...
GUIDANCE FOCUS:`;
    ...
    searchRelevantContent(query) {
        try {
            // Try multiple search strategies for better results
            let searchResults = (0, search_1.search)(query);
    ...
    tryAlternativeSearches(query) {
        ...
        for (const [topic, keywords] of Object.entries(searchMappings)) {
            if (keywords.some(keyword => lowerQuery.includes(keyword))) {
                const results = (0, search_1.search)(topic);
        ...
        for (const word of words) {
            const results = (0, search_1.search)(word);
            if (results && Array.isArray(results) && results.length > 0) {
                return results;
            }
        }
    ...
    getUserChatSessions(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                return yield chat_session_1.ChatSession.findByUser(userId);
            }
            catch (error) {
...
Summary:`;
                ...
                return ((_b = (_a = completion.choices[0]) === null || _a === void 0 ? void 0 : _a.message) === null || _b === void 0 ? void 0 : _b.content) || 'Previous conversation summary unavailable.';
            }
            catch (error) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            if (session.needsSummarization()) {
                ...
                if (messagesToSummarize.length > 0) {
                    const summary = yield this.createChatSummary(messagesToSummarize);
    ...
    manualSummarizeSession(userId, sessionId) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const session = yield chat_session_1.ChatSession.findOne({ userId, sessionId, isActive: true });
                if (!session) {
    ...
    getLearningRecommendations(user, course, progress) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a, _b;
...
STUDENT: ${(user === null || user === void 0 ? void 0 : user.fullName) || (user === null || user === void 0 ? void 0 : user.firstName) || 'Student'}${(user === null || user === void 0 ? void 0 : user.age) ? `, age ${user.age}` : ''}
PROGRESS: ${progress ? Math.round(progress.progress) : 0}% complete
CURRENT SECTION: ${(progress === null || progress === void 0 ? void 0 : progress.activeSection) || 'Beginning'}
...
Focus on GUIDANCE, not teaching. Be encouraging and actionable.`;
            try {
                ...
                const response = ((_b = (_a = completion.choices[0]) === null || _a === void 0 ? void 0 : _a.message) === null || _b === void 0 ? void 0 : _b.content) || 'I recommend continuing with the current section and practicing the concepts you\'ve learned.';
    ...
    analyzeLearningPatterns(user, course, progress) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a, _b;
...
COURSE: ${course.title}
STUDENT: ${(user === null || user === void 0 ? void 0 : user.fullName) || (user === null || user === void 0 ? void 0 : user.firstName) || 'Student'}
PROGRESS: ${progress ? Math.round(progress.progress) : 0}% complete
CURRENT SECTION: ${(progress === null || progress === void 0 ? void 0 : progress.activeSection) || 'Beginning'}
...
Focus on GUIDANCE and learning direction, not teaching content. Be encouraging and actionable.`;
            try {
                ...
                const response = ((_b = (_a = completion.choices[0]) === null || _a === void 0 ? void 0 : _a.message) === null || _b === void 0 ? void 0 : _b.content) || 'You\'re making great progress! Keep practicing regularly and don\'t hesitate to ask questions when you need help.';
                return this.parseAIResponse(response);
            }
            catch (error) {
